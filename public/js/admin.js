...
let repartidoresGlobales = []; // Caché local de repartidores
let repartidorEditandoId = null; // ID del repartidor en edición
...
document.getElementById('btnConfirmarBorrar').addEventListener('click', async function() {
    ...
    try {
        let url = '';
        if (tipoBorrado === 'negocio') url = `/api/negocios/${idParaBorrar}`;
        else if (tipoBorrado === 'cliente') url = `/api/clientes/${idParaBorrar}`;
        else if (tipoBorrado === 'repartidor') url = `/api/repartidores/${idParaBorrar}`;
        ...
        setTimeout(() => {
            if (tipoBorrado === 'negocio') {
                ...
                cargarClientes();
            } else if (tipoBorrado === 'repartidor') {
                cargarRepartidores();
            }
        }, 300);
...
async function cargarRepartidores() {
    try {
        ...
        repartidoresGlobales = repartidores;
        renderizarRepartidores(repartidoresGlobales);
    } catch (err) {
        console.error("Error al cargar repartidores", err);
    }
}

function renderizarRepartidores(repartidores) {
...
document.getElementById('buscadorRepartidores')?.addEventListener('input', (e) => {
    const query = e.target.value.toLowerCase().trim();
    const filtrados = repartidoresGlobales.filter(r => 
...
function abrirModalRepartidor(id = null) {
    repartidorEditandoId = id;
    ...
    if (id) {
        ...
        const rep = repartidoresGlobales.find(r => r.id === id);
        if (rep) {
...
document.getElementById('formEditarRepartidor')?.addEventListener('submit', async function(e) {
    ...
    const url = repartidorEditandoId ? `/api/repartidores/${repartidorEditandoId}` : '/api/repartidores';
    const method = repartidorEditandoId ? 'PUT' : 'POST';
